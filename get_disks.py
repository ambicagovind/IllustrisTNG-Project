...
def compare_mainseq(log_ms,log_sfr,z,snap_no):
    
    print('Comparing Main Sequence...')
    '''
    Compare how main sequence relations for different publications plot against TNG Data
    '''
    
    x = np.linspace(8, 11.5, len(log_ms))

    plt.figure(figsize=(8,6))
    plt.scatter(log_ms, log_sfr, s=1, label='Data')
    plt.plot(x,speagle(x,z), label='Speagle+14', linewidth=2, color='fuchsia')
    
    if z<0.1:
        plt.plot(x,chang(x), label='Chang+15', linewidth=2, color='darkorange')
    if z<0.085:
        plt.plot(x,renzini(x), label='Renzini&Peng15', linewidth=2, color='black')
    if ((z<2.5) & (z>0.5)):
        plt.plot(x,whittaker(x,z), label='Whitaker+14', linewidth=2, color='blue')
    if ((z<6) & (z>0.2)):
        plt.plot(x,pearson(x,z), label='Pearson+18', linewidth=2, color='brown')
    if z>0.5:
        plt.plot(x,iyer(x,z), label='Iyer+18', linewidth=2, color='plum')
        plt.plot(x,kurczynski(x,z), label='Kurczynski+16', linewidth=2, color='lawngreen')
    if z>1.3:
        plt.plot(x,santini(x,z), label='Santini+17', linewidth=2, color='deepskyblue')

    plt.xlabel('log Galaxy Stellar Mass [$M_\\odot$, $r < 2R_{eff}$]')
    plt.ylabel('log Star Formation Rate [yr$^{-1}$,$r < 2R_{eff}$]')
    plt.legend(frameon=False)
    
    plt.savefig(os.path.join(output,'{}'.format(snap_no)+'_compare.png'))
    print('{}'.format(snap_no)+'compare saved.')
...
def fit(z,log_ms_binned,log_sfr_binned, std_logsfr,log_ms,log_sfr, snap_no):
    
    print('Fitting..')
    
    '''
    Fit the binned(filtered) data to a straight line and compare with Speagle+14
    '''
    
    x = np.linspace(8, 11.5, len(log_ms))
    
    cs1=CubicSpline(x,speagle(x,z)+0.3)
    cs2=CubicSpline(x,speagle(x,z)-0.3)
    ...
    plt.plot(x,speagle(x,z), label='Speagle+14', linewidth=2, color='black')
...
for snap_no,rs in snaps[::-1]:
    ...
    compare_mainseq(log_ms,log_sfr,rs,snap_no)
    
    rs=np.round(z[0],3)

    good_ones=np.where((abs(speagle(log_ms,rs)-log_sfr)<0.6))
    fields=[ms,r,sfr,z,flags,ids,snaps1]
    newfields=[field[good_ones] for field in fields]
    ms,r,sfr,z,flags,ids,snaps1=newfields

    log_ms=np.log10(ms)
    log_sfr=np.log10(sfr)
    log_ms_binned,log_sfr_binned,std_logsfr=bins(log_ms,log_sfr)

    fit(rs,log_ms_binned,log_sfr_binned,std_logsfr, log_ms,log_sfr, snap_no) 
